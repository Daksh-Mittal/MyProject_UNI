...
bool validate_plot(mcpp::MinecraftConnection& mc, const mcpp::Coordinate& origin, const mcpp::Coordinate& bound, const std::vector<Plot>& existing_plots, const VillageConfig& config) {
    ...
    for (int x = plot_min_x; x <= plot_max_x; ++x) {
        for (int z = plot_min_z; z <= plot_max_z; ++z) {
            ...
            if (block == mcpp::BlockType(8) || block == mcpp::BlockType(9)) { 
                water_blocks++;
            }
        }
    }

    if (max_y - min_y > MAX_SLOPE_DELTA) {
        return false;
    }

    float current_water_coverage = (float)water_blocks / plot_area;
    if (current_water_coverage > MAX_WATER_COVERAGE) {
        return false;
    }
...
std::vector<Plot> find_plots(const VillageConfig& config) {
    mcpp::MinecraftConnection mc;
    std::vector<Plot> plots;
    
    int village_min_x = config.center_loc.x - config.village_size / 2;
    int village_min_z = config.center_loc.z - config.village_size / 2;
    int village_max_x = config.center_loc.x + config.village_size / 2;
    int village_max_z = config.center_loc.z + config.village_size / 2;

    int required_plots = std::max(1, config.village_size / 50); 
    const int MAX_ATTEMPTS = 1000;
    
    int plot_size_cycle = 14; // Start size for testmode
    
    // Set up the random engine for normal mode
    ...
    for (int attempt = 0; attempt < MAX_ATTEMPTS; ++attempt) {
        ...
        if (!validate_plot(mc, origin, bound, plots, config)) {
            continue;
        }
        ...
        mcpp::Coordinate entrance = determine_entrance(origin, bound, config);
...
mcpp::Coordinate determine_entrance(const mcpp::Coordinate& origin, const mcpp::Coordinate& bound, const VillageConfig& config) {
    ...
    int center_x = config.center_loc.x;
    int center_z = config.center_loc.z;
...
void terraform(const std::vector<Plot>& plots, const VillageConfig& config) {
    mcpp::MinecraftConnection mc;
    
    if (config.testmode) {
        std::cout << "--- TERRAFORMING START ---" << std::endl;
    }

    for (const auto& plot : plots) {
        int plot_h = plot.height;
        int border = config.plot_border;

        int min_x = std::min(plot.origin.x, plot.bound.x) - border;
        int max_x = std::max(plot.origin.x, plot.bound.x) + border;
        int min_z = std::min(plot.origin.z, plot.bound.z) - border;
        int max_z = std::max(plot.origin.z, plot.bound.z) + border;

        for (int x = min_x; x <= max_x; ++x) {
            for (int z = min_z; z <= max_z; ++z) {

                int dist_x = 0;
                if (x < std::min(plot.origin.x, plot.bound.x)) {
                    dist_x = std::min(plot.origin.x, plot.bound.x) - x;
                } else if (x > std::max(plot.origin.x, plot.bound.x)) {
                    dist_x = x - std::max(plot.origin.x, plot.bound.x);
                }

                int dist_z = 0;
                if (z < std::min(plot.origin.z, plot.bound.z)) {
                    dist_z = std::min(plot.origin.z, plot.bound.z) - z;
                } else if (z > std::max(plot.origin.z, plot.bound.z)) {
                    dist_z = z - std::max(plot.origin.z, plot.bound.z);
                }
    ...
    if (config.testmode) {
        std::cout << "--- TERRAFORMING END ---" << std::endl;
    }
}


void place_wall(const VillageConfig& config) {
    mcpp::MinecraftConnection mc;
    ...
    auto build_column = [&](int x, int z) {
        ...
        for (int y = 0; y < WALL_HEIGHT; ++y) {
            ...
            mc.setBlock(mcpp::Coordinate(x, surface_y + y + 1, z), WALL_MATERIAL); 
        }
        
        for (int y = surface_y + WALL_HEIGHT + 1; y < 256; ++y) {
            mc.setBlock(mcpp::Coordinate(x, y, z), mcpp::Blocks::AIR);
        }
    };
    ...
    if (config.testmode) {
        std::cout << "--- VILLAGE WALL END ---" << std::endl;
    }
}

std::vector<mcpp::Coordinate> find_waypoints(const std::vector<Plot>& plots, const VillageConfig& config) {
    mcpp::MinecraftConnection mc;
    ...
    auto is_inside_any_plot = [&](int x, int z) {
        for (const auto& plot : plots) {
            if (x >= std::min(plot.origin.x, plot.bound.x) && x <= std::max(plot.origin.x, plot.bound.x) &&
                z >= std::min(plot.origin.z, plot.bound.z) && z <= std::max(plot.origin.z, plot.bound.z)) {
                return true;
            }
        }
        return false;
    ...
    std::vector<std::pair<int, int>> potential_offsets = {
        {config.village_size / 4, config.village_size / 4},
        {-config.village_size / 4, config.village_size / 4},
        {config.village_size / 4, -config.village_size / 4},
        {-config.village_size / 4, -config.village_size / 4},
    };
